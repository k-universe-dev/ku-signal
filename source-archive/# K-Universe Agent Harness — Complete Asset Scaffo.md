...
# \# K-Universe Agent Harness — Complete Asset Scaffold (K2-Verified)

Generate a complete, exportable asset bundle for the K-Universe AI coding agent harness. This follows a K2 verification report that identified 3 critical blockers plus 1 architectural rule. Every artifact must be present. No omissions.

## K2 Verification Context (embedded — no external chat reference needed)

The following 3 blockers were identified in verification and must be reflected in generated code:

**BLOCKER 1 — src/protocol/events.ts**

- In `SessionUpdated` schema, `state` field must use `SessionStateSnapshotSchema`
- NEVER use `z.any()` for the state field
- Import: `import { SessionStateSnapshotSchema } from "./state.js"`

**BLOCKER 2 — src/core/agent.ts**
Interface `AgentCore` must include these exact signatures:

- `destroySession(sessionId: SessionId): Promise<void>` — must be idempotent (unknown sessionId must not throw)
- `cancelJob(jobId: JobId): Promise<void>` — must emit `JobCompleteEvent` with `successful: false` on completion
- `invokeTool(toolName: string, args: unknown): Promise<{ jobId: JobId }>` — returns jobId object, not void
- Also needs: `createSession(config: SessionConfig): Promise<SessionState>`
- Also needs: `executeCommand(cmd: Command): Promise<EventStream>`
- Import `JobId` from `"../protocol/state.js"`

**BLOCKER 3 — src/protocol/commands.ts**

- Add `protocolVersion: z.literal("1.0")` to `BaseCommand` interface
- Add `protocolVersion: z.literal("1.0")` field to EVERY `z.object()` block in CommandSchema

**ARCHITECTURAL RULE — src/core/models.ts**

- Type-only file. Zero provider SDK imports (no `import { OpenAI } from "openai"` or similar)
- Provider-specific implementations go in `src/core/models.impl.ts` or `src/providers/` factory modules only
- Interface file stays clean and UI-agnostic


## Output Format Rules

- Use exact file paths as headers: `### src/protocol/events.ts`
- Wrap each file in a markdown codeblock with correct language tag
- Generate Mermaid diagrams where architecture is explained
- Include data tables (markdown) for state matrices, command catalogs, event flows
- All Zod schemas must be complete and typed
...
File: `docs/FRD-001-Agent-Harness.md`
Sections:

- Purpose \& invariants (exactly 3)
- Subsystem decomposition (protocol, core, adapters, security)
- State machine diagram (Mermaid)
- Command/event matrix (table)
- Acceptance criteria per subsystem


### 2. Protocol Layer (.md specs + .ts implementation)

Files:

- `docs/protocol-spec.md` — complete command/event catalog with tables
- `src/protocol/commands.ts` — Zod schemas, all commands with `protocolVersion: z.literal("1.0")`
- `src/protocol/events.ts` — Zod schemas, `SessionUpdated` uses `SessionStateSnapshotSchema` (no `z.any()`)
- `src/protocol/state.ts` — discriminated unions, `SessionId`, `JobId`, `SessionStateSnapshotSchema`


### 3. Core Layer (.ts only)

File: `src/core/agent.ts`

- All 5 methods from Blocker 2 above
File: `src/core/models.ts`
- Type-only, zero provider SDK imports (Blocker rule)
- Factory pattern note: impls go in `src/core/models.impl.ts` or `src/providers/`


### 4. Adapter Layer (.ts stubs)

Files:

- `src/adapters/cli.ts` — stdin/stdout adapter stub
- `src/adapters/vscode.ts` — VS Code extension host stub
- `src/adapters/socket.ts` — WebSocket adapter stub
...
- `scripts/install.sh` — styled terminal installer. Use `@clack/prompts` or `gum` style. Reference aesthetic: bordered box with gradient header, step progress with spinners/checkmarks, footer with version+URL. Think: clean TUI install screen.
- `scripts/scaffold.ts` — project bootstrap (creates dirs, copies templates)
- `scripts/verify.ts` — K2-style verification runner. Checks: no `z.any()` in events.ts, protocolVersion present in all commands, models.ts has zero provider imports, agent.ts has all 5 required methods with exact signatures
...
- `docs/ADR-001-Protocol-Design.md` — why discriminated unions + Zod over classes
- `docs/ADR-002-Session-Lifecycle.md` — state machine rationale with Mermaid diagram
- `docs/research/model-provider-matrix.md` — table: provider | context window | cost | integration status


### 7. Charts \& Tables (inline in docs)

- State transition diagram (Mermaid flowchart TD)
- Command → Event mapping table
- Directory tree diagram (Mermaid)
- Security boundary diagram (Mermaid with subgraphs)
...
- `docs/runbooks/debugging.md` — systematic debugging workflow (root cause → defense-in-depth → verification)
- `docs/runbooks/deployment.md` — deploy checklist
- `notes/k2-verification-report.md` — embedded findings (the 3 blockers + architectural rule above) with fix status


## Invariant Checks (must pass in generated code)

1. `src/protocol/events.ts`: `state` field uses `SessionStateSnapshotSchema`, never `z.any()`
2. `src/core/agent.ts`: `destroySession`, `cancelJob`, `invokeTool` signatures exact as specified in Blocker 2
3. `src/protocol/commands.ts`: every command schema includes `protocolVersion: z.literal("1.0")`
4. `src/core/models.ts`: zero provider SDK imports
5. `src/core/models.ts`: type-only exports, impls live in factories

## Export Structure

Generate all files in a single response, grouped by category. Prefix each group with a `##` header. Include a final `## Asset Manifest` section listing every file path generated.

Do not summarize. Do not omit files for brevity. Output the complete bundle.
...
| Command Type | Emits (success) | Emits (failure) |
| :-- | :-- | :-- |
| `CreateSession` | `SessionCreated`, `SessionUpdated` | `ProtocolError` |
| `ExecuteCommand` | `JobStarted`, `JobProgress*`, `JobComplete` | `JobComplete(successful:false)` |
| `InvokeTool` | `JobStarted`, `ToolResult`, `JobComplete` | `JobComplete(successful:false)` |
| `CancelJob` | `JobComplete(successful:false)` | *(no-op if job not found)* |
| `DestroySession` | `SessionDestroyed` | *(no-op if session not found)* |
| `UpdateContext` | `SessionUpdated` | `ProtocolError` |
| `ListTools` | `ToolListResult` | `ProtocolError` |
...
| \# | File Path | Category |
| :-- | :-- | :-- |
| 1 | `docs/FRD-001-Agent-Harness.md` | FRD |
| 2 | `docs/protocol-spec.md` | Protocol Spec |
| 3 | `src/protocol/state.ts` | Protocol Layer |
| 4 | `src/protocol/commands.ts` | Protocol Layer |
| 5 | `src/protocol/events.ts` | Protocol Layer |
| 6 | `src/core/agent.ts` | Core Layer |
| 7 | `src/core/models.ts` | Core Layer |
| 8 | `src/adapters/cli.ts` | Adapter Layer |
| 9 | `src/adapters/vscode.ts` | Adapter Layer |
| 10 | `src/adapters/socket.ts` | Adapter Layer |
| 11 | `scripts/install.sh` | Scripts |
| 12 | `scripts/scaffold.ts` | Scripts |
| 13 | `scripts/verify.ts` | Scripts |
| 14 | `docs/ADR-001-Protocol-Design.md` | Design Notes |
| 15 | `docs/ADR-002-Session-Lifecycle.md` | Design Notes |
| 16 | `docs/research/model-provider-matrix.md` | Research |
| 17 | `docs/runbooks/debugging.md` | Runbooks |
| 18 | `docs/runbooks/deployment.md` | Runbooks |
| 19 | `notes/k2-verification-report.md` | K2 Report |
